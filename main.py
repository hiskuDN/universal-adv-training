...
parser.add_argument('--dataset', type=str, default='mnist',
                    help="mnist | svhn | celebA")
parser.add_argument('--adv', action='store_true',
                    help="using adversarially trained network")
parser.add_argument('--classifier', type=str, default='resnet',
                    help='resnet | vgg | madry | aditi | zico')
parser.add_argument('--datapath', type=str,
                    default=None, help="input data path")
parser.add_argument('--seed', type=int, default=1234, help="random seed")
parser.add_argument('--batch_size', type=int, default=64, help="batch size")
parser.add_argument('--mode', type=str, default='attack',
                    help='eval | targeted_attack | untargeted_attack')
parser.add_argument('--top5', action='store_true', help="use top5 error")

parser.add_argument('--lr', type=float, default=1,
                    help="learning rate for doing targeted/untargeted attack")
...
parser.add_argument('--source', type=int, default=0,
                    help="ground truth class (source class)")
...
parser.add_argument('--noise', action="store_true",
                    help="add noise augmentation to attacks")
parser.add_argument('--z_eps', type=float, default=0.1,
                    help="soft constraint for the search region of latent space")
parser.add_argument('--adv_gen', action="store_true",
                    help="adversarial training using generative adversarial examples")
parser.add_argument('--trained', action="store_true", help="trained models")
parser.add_argument('--test_num', type=int, default=0,
                    help="test iteration version")
...
def vgg_template(images, training, hps):
    images_standardized = per_image_standardization(images)
    logits, _ = vgg_16(images_standardized, num_classes=hps.num_classes,
                       is_training=training, dataset=hps.dataset)
    return logits
...
def evaluate(hps, data_X, data_y, eval_once=True):
    ...
    images = tf.placeholder(tf.float32, shape=(
        None, args.image_size, args.image_size, args.channels))
    ...
    adv_image = fgm(images, tf.nn.softmax(logits), y=labels_onehot,
                    eps=args.eps / 10, clip_min=0.0, clip_max=1.0)
    ...
    while True:
        ...
        for batch in range(num_batch):
            ...
            correct_prediction += np.sum(y ==
                                         predictions) if not args.top5 else np.sum(in_top5)
        ...
        bad_images = np.reshape(
            bad_images, (len(bad_images) * args.image_size, args.image_size, args.channels))
        bad_images = np.squeeze(bad_images)

        imageio.imwrite(os.path.join(check_folder(
            'tmp'), 'bad_images.png'), bad_images)
        print("bad_labels:\n{}".format(bad_labels))
...
def untargeted_attack(hps, lambda1, lambda2, source, noise=False):
    ...
    for i in range(args.num_classes):
        if i == source:
            continue
        onehot = np.zeros(
            (args.batch_size, args.num_classes), dtype=np.float32)
        onehot[:, i] = 1.0
        obj_classes.append(tf.nn.softmax_cross_entropy_with_logits(
            logits=net_logits, labels=onehot))

    all_cross_entropy = tf.stack(obj_classes, axis=1)
    min_cross_entropy = tf.reduce_mean(
        tf.reduce_min(all_cross_entropy, axis=1))
    ...
    with tf.variable_scope("train_ops"):
        ...
        for grad, v in grads:
            if v is not adv_noise:
                new_grads.append(
                    (grad + tf.random_normal(shape=grad.get_shape().as_list(), stddev=tf.sqrt(var)), v))
            else:
                new_grads.append(
                    (grad / tf.sqrt(tf.reduce_sum(tf.square(grad), axis=[1, 2, 3], keep_dims=True)), v))

        adv_op = optimizer.apply_gradients(new_grads)

    momentum_init = tf.variables_initializer(tf.get_collection(
        tf.GraphKeys.GLOBAL_VARIABLES, scope='train_ops'))
    init_op = tf.group(
        momentum_init, tf.variables_initializer([adv_z, adv_noise]))
    ...
    if args.classifier == 'madry':
        if args.trained:
            saver4classifier = tf.train.Saver(tf.get_collection(
                tf.GraphKeys.TRAINABLE_VARIABLES, scope="net"))
        else:
            saver4classifier = tf.train.Saver(
                {x.name[4:-2]: x for x in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="net")})
    else:
        saver4classifier = tf.train.Saver(var_list=tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope='net'))

    checkpoint_dir = os.path.join(
        args.checkpoint_dir, acgan.model_dir, acgan.model_name)
    try:
        ckpt_state = tf.train.get_checkpoint_state(save_path)
    except tf.errors.OutOfRangeError as e:
        print('[!] Cannot restore checkpoint: %s' % e)
    ...
    for batch in range(args.n_adv_examples):
        ...
        sess.run(init_op)
        preds_np, probs_np, im_np, cost_before = sess.run(
            [net_pred, net_softmax, adv_images, obj])

        # Using GD for attacking
        # initialize optimizers
        for i in range(args.n_iters):
            _, now_cost, pred_np, acgan_pred_np = sess.run(
                [adv_op, obj, net_pred, acgan_pred], feed_dict={_iter: i})
            ok = np.logical_and(pred_np != source, acgan_pred_np == source)
            print("   [*] {}th iter, cost: {}, success: {}/{}".format(i +
                  1, now_cost, np.sum(ok), args.batch_size))
        ...
        if len(adv_im_np) >= args.n2collect:
            ...
            img = label_images(
                adv_im_np[:args.n2collect, ...], adv_labels_np[:args.n2collect])
...
def defense_by_attack(hps, lambda1=100, lambda2=20, noise=False, test_num=0):
    ...
    if args.dataset == 'mnist':
            ...
            for target in range(10):
                if(source != target):
                    source_np = np.asarray(
                        [source] * args.batch_size).astype(np.int32)
                    target_np = np.asarray(
                        [target] * args.batch_size).astype(np.int32)
                    ...
                    if noise:
                        ...
                        if args.dataset in ('svhn', 'celebA'):
                            adv_images = tf.clip_by_value(
                                adv_images, clip_value_min=-1., clip_value_max=1.)
                        else:
                            adv_images = tf.clip_by_value(
                                adv_images, clip_value_min=0., clip_value_max=1.0)
                    ...
                    if not (ckpt_state and ckpt_state.model_checkpoint_path):
                        print('[!] No model to eval yet at %s' % save_path)
                    print('Checkpoint path: %s' % ckpt_state.model_checkpoint_path)
                    print('[*] Loading checkpoint %s' %
                        ckpt_state.model_checkpoint_path)
                    saver4classifier.restore(
                    ...
                    for batch in range(args.n_adv_examples):
                        ...
                        for i in range(args.n_iters):
                            _, now_cost, pred_np, acgan_pred_np, acgan_probs = sess.run(
                                ...
                                feed_dict={_iter: i})
                            ok = np.logical_and(
                                pred_np == target, acgan_pred_np == source)
                            print("   [*] {}th iter, cost: {}, success: {}/{}".format(i +
                                                                                    1, now_cost, np.sum(ok), args.batch_size))
                        ...
                        acc += np.sum(preds_np == source)
                        idx = np.logical_and(
                            adv_preds_np == target, acgan_preds_np == source)
                        adv_acc += np.sum(idx)
                        adv_im_np.extend(im_np[idx])
                        latent_z.extend(hidden_z[idx])
        ...
        if args.classifier == 'madry':
            if args.trained:
                saver4classifier = tf.train.Saver(tf.get_collection(
                    tf.GraphKeys.TRAINABLE_VARIABLES, scope="init"))
            else:
                saver4classifier = tf.train.Saver(
                    {x.name[4:-2]: x for x in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="init")})
        else:
            saver4classifier = tf.train.Saver(var_list=tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope='init'))
...
def main():
    ...
    if args.dataset == 'mnist':
        ...
        if args.mode == 'eval':
            data_X, data_y, test_X, test_y = load_mnist4classifier(
                args.dataset)
    ...
    if args.mode == 'eval':
        ...
        targeted_attack(hps, args.source, args.target,
                        args.lambda1, args.lambda2, noise=args.noise)
    elif args.mode == 'untargeted_attack':
        untargeted_attack(hps, args.lambda1, args.lambda2,
                          source=args.source, noise=args.noise)
    elif args.mode == 'defense_by_attack':
        defense_by_attack(hps, args.lambda1, args.lambda2,
                          noise=args.noise, test_num=args.test_num)
    else:

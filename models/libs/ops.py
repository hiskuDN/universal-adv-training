...
def linear(input_, output_size, name="linear", spectral_normed=False, update_collection=None, stddev=None,
...
def gan_cond_batch_norm(input, labels, n_labels=10, name="cond_batch_norm"):
    '''
    Batch normalization in GANs is different. Do not use running statistics for testing
    '''
    import tensorflow.compat.v1 as tf
    tf.disable_v2_behavior()
    mean, var = tf.nn.moments(input, axes=[0, 1, 2], keepdims=True)
    shape = mean.get_shape().as_list()
    offset_m = tf.get_variable(name+'_offset', shape=(n_labels, shape[-1]), dtype=tf.float32,
                               initializer=tf.zeros_initializer)
    scale_m = tf.get_variable(name+'_scale', shape=(n_labels, shape[-1]), dtype=tf.float32,
                              initializer=tf.ones_initializer)
    offset = tf.nn.embedding_lookup(offset_m, labels)
    scale = tf.nn.embedding_lookup(scale_m, labels)

    result = tf.nn.batch_normalization(input, mean, var, offset[:, None, None, :], scale[:, None, None, :], 1e-5)
    return result

def gan_batch_norm(input, name="batch_norm"):
    '''
    Batch normalization in GANs is different. Do not use running statistics for testing
    '''
    import tensorflow.compat.v1 as tf
    ...
    mean, var = tf.nn.moments(input, axes=[0, 1, 2], keepdims=True)
    shape = mean.get_shape().as_list()
    offset = tf.get_variable(name+'_offset', shape=shape, dtype=tf.float32, initializer=tf.zeros_initializer)
    scale = tf.get_variable(name+'_scale', shape=shape, dtype=tf.float32, initializer=tf.ones_initializer)

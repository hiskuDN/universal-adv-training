...
class ZicoCIFAR10(object):
    def __init__(self, images):
        ...
        y = tf.pad(self.x_image, [[0, 0], [1, 1], [1, 1], [0, 0]])  # [batch_size, 34, 34, 3]
        y = tf.nn.conv2d(y, W0, strides=[1, 2, 2, 1], padding='VALID')  # [batch_size, 16, 16, 16]
        y = tf.nn.bias_add(y, B0)
        y = tf.nn.relu(y)
        
        y = tf.pad(y, [[0, 0], [1, 1], [1, 1], [0, 0]])  # [batch_size, 18, 18, 16]
        y = tf.nn.conv2d(y, W2, strides=[1, 2, 2, 1], padding="VALID")  # [batch_size, 8, 8, 32]
        y = tf.nn.bias_add(y, B2)
        y = tf.nn.relu(y)
        
        y = tf.transpose(y, [0, 3, 1, 2])
        y = tf.reshape(y, [tf.shape(y)[0], -1])  # Flatten to [batch_size, 2048]
        y = tf.matmul(y, W5) + B5
        y = tf.nn.relu(y)

        # Output layer
        logits = tf.matmul(y, W7) + B7  # Output logits [batch_size, 2]
        self.logits = logits  # Logits before softmax
        self.probabilities = tf.nn.softmax(logits)  # Apply softmax activation]
